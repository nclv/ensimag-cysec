...
static inline uint64_t __my_little_xoshiro256starstar__rotl(const uint64_t x,
															int k) {
	return (x << k) | (x >> (64 - k));
}
...
void __my_little_xoshiro256starstar_unseeded_init() {
	...
	if (urd == NULL) {
		fprintf(stderr, "failed to initialize the little xoshiro256** prng [No "
						"file called /dev/urandom]\n");
		__my_little_xoshiro256starstar_initialization(iv);
		return;
	}

	if (1 != fread((uint8_t *)iv, 32, 1, urd)) {
		fprintf(stderr, "failed to initialize the little8 xoshiro256** prng "
						"[Not enough p$ bytes]\n");
	}
	fclose(urd);
	__my_little_xoshiro256starstar_initialization(iv);
...
uint64_t __my_little_xoshiro256starstar__next__unsafe(void) {
	const uint64_t result_starstar =
		__my_little_xoshiro256starstar__rotl(
			__my_little_xoshiro256starstar__s[1] * 5, 7) *
		9;

	const uint64_t t = __my_little_xoshiro256starstar__s[1] << 17;

	__my_little_xoshiro256starstar__s[2] ^=
		__my_little_xoshiro256starstar__s[0];
	__my_little_xoshiro256starstar__s[3] ^=
		__my_little_xoshiro256starstar__s[1];
	__my_little_xoshiro256starstar__s[1] ^=
		__my_little_xoshiro256starstar__s[2];
	__my_little_xoshiro256starstar__s[0] ^=
		__my_little_xoshiro256starstar__s[3];
	...
	__my_little_xoshiro256starstar__s[3] = __my_little_xoshiro256starstar__rotl(
		__my_little_xoshiro256starstar__s[3], 45);
...
/* This is the jump function for the generator. It is equivalent
   to 2^128 calls to __my_little_xoshiro256starstar__next(); it can be used to
...
/* This is the long-jump function for the generator. It is equivalent to
   2^192 calls to __my_little_xoshiro256starstar__next(); it can be used to
   generate 2^64 starting points, from each of which jump() will generate 2^64
